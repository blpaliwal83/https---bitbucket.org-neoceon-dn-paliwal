...
var openFB = (function () {
    ...
    var loginURL = 'https://www.facebook.com/v2.0/dialog/oauth',
     ...
     * @returns {*}
     */
    function login(clientId, appScope, callback, options) {
    ...
    if (window.cordova) {
        ...
        var flowUrl = "https://www.facebook.com/v2.5/dialog/oauth?client_id=" + clientId + "&redirect_uri=" + redirect_uri + "&response_type=token&scope=" + appScope.join(",");
        ...
        browserRef.addEventListener('loadstart', function(event) {
          if((event.url).indexOf(redirect_uri) === 0) {
            ...
            if(parameterMap.access_token !== undefined && parameterMap.access_token !== null) {
                tokenStore.fbAccessToken = parameterMap.access_token;
                loginCallback({status: 'connected',authResponse: { access_token: parameterMap.access_token, expires_in: parameterMap.expires_in }})
              ...
              if ((event.url).indexOf("error_code=100") !== 0) {
                loginCallback("Facebook returned error_code=100: Invalid permissions");
              } else {
                loginCallback("Problem authenticating");
              }
            }
          }
        });
        browserRef.addEventListener('exit', function(event) {
          loginCallback("The sign in flow finished");
        });
    } else {
      loginCallback("Cannot authenticate via a web browser");
    }
    return;
        // var loginWindow,
        //     startTime,
